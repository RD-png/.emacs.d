...
Think of these like sessions, where you can open multiple buffers and group them into a perspective
...
Currently using this for both .vue and .ts files, also provies completion and error checking via lsp server
...
Currently not using vue mode as it is fairly slow, but will save my configuration for it here if it sees any updates might give it another try
...
Remove white space from current buffer on save
...
Run garbage collection more often but in less time
